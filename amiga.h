...
/* rastport draw modes: (source: http://wiki.amigaos.net/wiki/Graphics_Primitives )
JAM1
    Whenever you execute a graphics drawing command, one color is jammed into the target drawing area.
    You use only the primary drawing pen color, and for each pixel drawn, you replace the color at that location with the FgPen color.
tl;dr: font is drawn without background (i.e with transparency)
JAM2
    Whenever you execute a graphics drawing command, two colors are jammed into the target drawing area.
    This mode tells the system that the pattern variables (both line pattern and area pattern – see the next section) are to be used for the drawing.
    Wherever there is a 1 bit in the pattern variable, the FgPen color replaces the color of the pixel at the drawing position.
    Wherever there is a 0 bit in the pattern variable, the BgPen color is used.
td;r: font area gets filled with bgcolor, and the font painted on top.
COMPLEMENT
